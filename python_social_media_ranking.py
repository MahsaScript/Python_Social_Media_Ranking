# -*- coding: utf-8 -*-
"""Python_Social_Media_Ranking.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/19c9a0l7qakgvjvSpe4445lDqRNZaGibM

#Google Trends - Install pytrends
"""
...
keywords = pytrend.suggestions(keyword='university ')
df = pd.DataFrame(keywords)
df.head(5)

#get today's treniding topics
#trendingtoday = pytrend.today_searches(pn='IR')
#trendingtoday.head(20)

# Create payload and capture API tokens. Only needed for interest_over_time(), interest_by_region() & related_queries()
pytrend.build_payload(kw_list=['university ', 'university '])
...
url='https://www.webometrics.info/en/'
...
divs = soup.findAll("table", {"class": "sticky-enabled"})
for div in divs:
    row = ''
    rows = div.findAll('tr')
    for row in rows:
        if(row.text.find("University of Tehran") > -1):
            s=row.text
            print("University of Tehran: " + s[1:4])
        if(row.text.find(" University of Medical Sciences") > -1):
            s=row.text
            print(" University of Medical Sciences: " + s[1:4])

"""#Alexa Ranking"""

url = 'https://www.mastersportal.com/ranking-country/110/iran.html'
with open("alexa.txt", "rb") as f:
    text= f.read()
#print(text)
...
import re
soup = BeautifulSoup(text, 'html.parser')
divs = soup.findAll("table", {"class": "RankingTable"})
for div in divs:
    row = ''
    rows = div.findAll('tr')
    for row in rows:
        if(row.text.find(" University of Technology") > -1):
            s=row.text
            print(s)

"""#Iranian Social Media Ranking  - Instagram"""

url = 'https://www.4icu.org/instagram/ir/'
with open("social.txt", "rb") as f:
    text= f.read()
#print(text)
...
import re
soup = BeautifulSoup(text, 'html.parser')
divs = soup.findAll("table", {"class": "table table-hover text-left"})
for div in divs:
    row = ''
    rows = div.findAll('tr')
    for row in rows:
        if(row.text.find(" University of Medical Sciences") > -1):
            s=row.text
            print("University of Medical Sciences (member/ posts ):"+s)

"""#Telegram - Install telethon"""
...
"""##Getting api_code and api_hash from https://my.telegram.org/"""
...
print('University Name / Channel Name / Members')
print('اعضا / کانال /دانشگاه   ')
